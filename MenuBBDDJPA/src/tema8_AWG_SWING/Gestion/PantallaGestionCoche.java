...
public class PantallaGestionCoche extends JPanel {
	...
	JComboBox<Fabricante> jcIdFabricante = new JComboBox<Fabricante>();
	JTextField jtfBastidor = new JTextField(20);
	JTextField jtfModelo = new JTextField(20);
	...
	private void construir () {
		//creamos lista de fabricantes y la añadimos al combobox
		List<Fabricante> fabri = FabricanteControlador.getControlador().findAll();
		
		for(Fabricante f : fabri) {
			jcIdFabricante.addItem(f);
		}
		...
		// Inclusiï¿½n del campo "ID Fabricante"
		c.gridx = 0;
	    c.gridy = 2;
	    c.anchor = GridBagConstraints.EAST;
	...
	private void limpiarPantalla() {
		this.jtfId.setText("");
		this.jcIdFabricante.setSelectedIndex(-1);
	...
	private void guardar () {
		...
		nuevoRegistro.setFabricante((Fabricante) this.jcIdFabricante.getSelectedItem());;
	...
	private void cargarDatosActual () {
		if (this.actual != null) {
			...
			this.jcIdFabricante.setSelectedItem((this.actual.getFabricante()));
